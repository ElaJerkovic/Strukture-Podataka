...
int countryFind(lista HashTablica[], position2 head, char* country, int number);
int fileRead(char* name, position2 head, lista HashTable[]);
...
position treeFileRead(char* name, position root);
...
int initializeList(position2 head);
position cityFind(position current, int number);

int main()
{
  	lista head = { .country_name = { 0 },.root = NULL,.next = NULL };
	position2 p = &head;
	char ime[MAX_SIZE] = { 0 };
	int number;
	...
	fileRead("drzave.txt", p, &HashTablica);
	...
	countryFind(HashTablica, p, ime, number);

  	return 0;
}

int fileRead(char* name, position2 head, lista HashTablica[])
{
  	char buffer[MAX_LINE] = { 0 };
	char country_name[MAX_LINE] = { 0 };
	FILE* fp = NULL;
  	...
  	while (!feof(fp))
	  {
		...
		if (sscanf(buffer, " %s %s", newEl->country_name, country_name) == 2)
	    	{
		       	newEl->root = NULL;
                newEl->root = treeFileRead(country_name, newEl->root);
		      	insertSorted(newEl, head);
		      	HashTable(HashTablica, newEl);
		 }
   	}
  	fclose(fp);
  	return 0;
...
int initializeList(position2 head)
{
	memset(head->country_name, 0, MAX_SIZE);
	head->root = NULL;
	head->next = NULL;
...
int insertSorted(position2 newEl, position2 head)
{
	    ...
	    while (temp->next != NULL && strcmp(temp->next->country_name, newEl->country_name) < 0)
		  temp = temp->next;
...
position insertToTree(position newEl, position current)
{
	    ...
	    {
            if (strcmp(current->city_name, newEl->city_name) < 0)
                current->right = insertToTree(newEl, current->right);

            else if (strcmp(current->city_name, newEl->city_name) > 0)
                current->left = insertToTree(newEl, current->left);
...
int printTree(position current)
{
	 ...
	 printf("%s %d\n", current->city_name, current->population);
...
int HashTable(lista HashTablica[],position newElement)
{
	int num = HashNumber(newElement->city_name);

	insertSorted(newElement,&HashTablica[num]);
...
int countryFind(lista HashTablica[], position2 head, char* country, int number)
{
	  ...
	  while (temp != NULL && strcmp(country, temp->country_name) != 0)
	  	  temp = temp->next;
